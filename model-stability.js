...
function onModelRoute() {
  return (location.hash.slice(1) || 'home') === 'models'
}
...
function restoreModelContext() {
  restoreFrame = 0
  if (!restoreContext || !onModelRoute() || Date.now() - restoreContext.createdAt > 65000) return
...
appRoot.addEventListener('click', (event) => {
  if (!onModelRoute() || !event.target.closest('#ai-settings-form')) return
...
addEventListener('hashchange', () => {
  ...
  queueMicrotask(stabilizeModelPage)
})

new MutationObserver(() => queueMicrotask(stabilizeModelPage)).observe(appRoot, {
  childList: true,
  subtree: true,
})
